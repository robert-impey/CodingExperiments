...
object Newton {;import org.scalaide.worksheet.runtime.library.WorksheetSupport._; def main(args: Array[String])=$execute{;$skip(39); val res$0 = 
  ...
  def abs(x: Double): Double = if (x < 0) -x else x;System.out.println("""abs: (x: Double)Double""");$skip(90); 
  def isGoodEnough(guess: Double, x: Double): Boolean =
    abs(guess * guess - x) < 1e-3;System.out.println("""isGoodEnough: (guess: Double, x: Double)Boolean""");$skip(80); 
